...
class CorralSmoothIGW:
    ...
    def sample(self, fhat, X, fhatstar, ahatstar):
        ...
        self.gamma = self.gscale(self.gzero,self.T)
        ...
        probs = tau / (tau + self.gamma*(fhatrando-fhatstar))        
...
class CorralCappedIGW:
    ...
    def sample(self, fhat, X, fhatstar, ahatstar):
        ...
        taus     = torch.gather(input=self.taus.unsqueeze(0).expand(N, -1), dim=1, index=algos.unsqueeze(1))
        ...
        for tau,x in zip(taus,X):

            beta = self.find_beta_martingale(fhat,self.gamma,x,tau.item())
            m_t  = lambda a: 1/(self.lamb+self.gamma*torch.clamp(fhat(x,a)-beta,min=0))
    ...
    def find_beta_martingale(self,fhat,gamma,x,tau):
        ...
        #So, it is g = lambda f,beta: tau/max(<self.lamb+gamma*(fhat-beta)>,self.lamb)
        #we do it this crazy kind of way to reduce the repeated calculations in g
        ...
        def batched_base_action_sampler():
            while True:
                yield from fhat(x,self.base_action_sampler(1000)).squeeze().tolist()
